...
next.addEventListener('click', () => {
	...
	if (offset == +width.slice(0, width.length - 2) * (sliders.length - 1)) { // '500px' -- если ето последний слайд
		offset = 0;
	} else {

		offset += +width.slice(0, width.length - 2);// не последний слайд то добавляем смещеніе
	}
	...
	// установка текущего индекса в блоке на странице
		if(sliders.length < 10){
			current.textContent = `0${indexSlider}`;
		} else {
			current.textContent = indexSlider;
